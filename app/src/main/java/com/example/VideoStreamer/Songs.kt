...
class Songs : Fragment() {
    ...
    override fun onCreateView(
        ...
        var layout = inflater.inflate(R.layout.fragment_songs, container, false)
        val connectivityManager =
            layout.context.getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager
        val activeNetwork: NetworkInfo? = connectivityManager.activeNetworkInfo
        val isConnected: Boolean = activeNetwork?.isConnectedOrConnecting == true

        var recyclerView = layout.findViewById<RecyclerView>(R.id.song_recycler_view)
        var progressBar = layout.findViewById<ProgressBar>(R.id.songProgressBar)
        var coming_soon = layout.findViewById<ImageView>(R.id.comingsoon_songs)
        if(isConnected==false) {
            Toast.makeText(context,"You are not connected to the internet", Toast.LENGTH_SHORT).show()
            progressBar.visibility=View.GONE
        }
        databaseReference = FirebaseDatabase.getInstance().reference.child("songs")
        if(isConnected==false) {
