 ...
 * Use the [web_series.newInstance] factory method to
 * create an instance of this fragment.
 */
class web_series : Fragment() {
    ...
    override fun onCreateView(
        ...
        var layout = inflater.inflate(R.layout.fragment_web_series, container, false)
        val connectivityManager =
            layout.context.getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager
        val activeNetwork: NetworkInfo? = connectivityManager.activeNetworkInfo
        val isConnected: Boolean = activeNetwork?.isConnectedOrConnecting == true

        var recyclerView = layout.findViewById<RecyclerView>(R.id.webseries_recycler_view)
        var progressBar = layout.findViewById<ProgressBar>(R.id.webseriesProgressBar)
        var coming_soon = layout.findViewById<ImageView>(R.id.comingsoon_webseries)
        if(isConnected==false) {
            Toast.makeText(context,"You are not connected to the internet", Toast.LENGTH_SHORT).show()
            progressBar.visibility=View.GONE
        }
        databaseReference = FirebaseDatabase.getInstance().reference.child("webseries")
        if(isConnected==false) {
    ...
    companion object {
         ...
         * @return A new instance of fragment web_series.
         */
        @JvmStatic
